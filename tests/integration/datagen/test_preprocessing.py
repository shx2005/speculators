...
@pytest.mark.sanity
def test_preprocess_batch_fails_fast_on_processing_error(monkeypatch):
    """Preprocessing errors should not silently drop samples from a batch."""
    examples = {
        "messages": [
            [
                {
    ...
    def raise_image_error(_image_ref):
        raise OSError("missing image")

    monkeypatch.setattr(
        "speculators.data_generation.preprocessing._load_image_for_processor",
        raise_image_error,
    )

    with pytest.raises(RuntimeError, match="Failed to process conversation 0"):
        _preprocess_batch(
            examples,
            cast("PreTrainedTokenizerBase", _DummyMultimodalProcessor()),
            max_length=64,
            assistant_pattern=None,
        )
